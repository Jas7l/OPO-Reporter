...
class ScheduleAdjustmentService:
    ...
    def _serialize(self, adj: ScheduleAdjustment) -> Dict[str, Any]:
        ...
        return {
            ...
            'status_override': (
                adj.status_override.value if adj.status_override else None
            ),
            'start_time_override': (
                adj.start_time_override.strftime('%H:%M')
                if adj.start_time_override else None
            ),
            'end_time_override': (
                adj.end_time_override.strftime('%H:%M')
                if adj.end_time_override else None
            ),
            'lunch_start_override': (
                adj.lunch_start_override.strftime('%H:%M')
                if adj.lunch_start_override else None
            ),
            'absences': adj.absences if adj.absences else [],
            'created_at': (
                adj.created_at.isoformat() if adj.created_at else None
            ),
        }
    ...
    def create_adjustment(self) -> Dict[str, Any]:
        ...
        try:
            status_enum = EmployeeStatusCode(status_override) \
        ...
        with self._pg.begin():
            ...
            self._logger.debug(
                'Правка создана', extra={'id': db_adjustment.id}
            )
            ...
            if 'status_override' in data:
                try:
                    ...
                    adjustment.status_override = (
                        EmployeeStatusCode(val) if val else None
                    )
                except ValueError:
                    raise ModuleException(
